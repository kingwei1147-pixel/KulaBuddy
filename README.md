# KulaBuddy — 本地优先自主 Agent / Local-First Autonomous Agent

[中文](#中文) | [English](#english)

KulaBuddy is an autonomous AI agent with **built-in GGUF runtime**, multi-model orchestration, automation, and self-learning. No Ollama, no vLLM, no LM Studio required — load local GGUF models directly, or plug into any OpenAI-compatible cloud API.

KulaBuddy 是一个 **本地大模型直载 + 外部大模型接入** 的自主 AI Agent。不依赖 Ollama/vLLM/LM Studio，可直接加载 GGUF 模型运行，也可接入 OpenAI-compatible 云端接口。原生中文支持，全栈覆盖从输入到执行到自进化的完整闭环。

---

## 为什么选择 KulaBuddy / Why KulaBuddy

KulaBuddy 的独特定位：**全栈自主 + 本地优先 + 中文原生 + MCP 自配置**，填补了现有 agent 之间的空白。

### 竞品对比 / Competitor Comparison

| 维度 Dimension | KulaBuddy | Claude Code | OpenClaw | Hermes Agent |
|:---|:---|:---|:---|:---|
...
### KulaBuddy 的护城河 / KulaBuddy's Unique Advantages
...
1. 双击 `install-kulabuddy.bat` 安装依赖并构建
2. 双击 `daDa.bat` 启动 KulaBuddy 控制台
3. 浏览器打开 [http://localhost:9877](http://localhost:9877)

### Recommended: One-Click Start (Windows)

1. Double-click `install-kulabuddy.bat` to install & build
2. Double-click `daDa.bat` to launch KulaBuddy
